...
use crate::retry::RetryTokenizer;
...
impl NetworkRuntime {
    pub fn start(bind_addr: SocketAddr) -> Result<Self> {
        ...
        let retry_tokenizer = Arc::new(RetryTokenizer::new());
        ...
        for worker_id in 0..worker_count {
            ...
            let worker_retry_tokenizer = Arc::clone(&retry_tokenizer);
...
fn run_io_thread(
    ...
    retry_tokenizer: Arc<RetryTokenizer>,
) -> Result<()> {
    ...
    while running.load(Ordering::Relaxed) {
        ...
        // Hand off any session-less datagrams (e.g. QUIC Retry packets) to the send queue.
        ready_datagrams.extend(shard.outgoing_datagrams.drain(..));
...
struct ShardState {
    ...
    event_tx: mpsc::Sender<NetworkEvent>,
    next_client_id: Arc<AtomicU32>,
    outgoing_datagrams: VecDeque<PacedDatagram>,
}
...
fn handle_add_connection(shard: &mut ShardState, client_addr: SocketAddr, initial_packet: Vec<u8>) {
    ...
    let (header_version, header_scid, header_dcid, header_token) =
        match quiche::Header::from_slice(&mut pkt_buf, quiche::MAX_CONN_ID_LEN) {
            Ok(h) => {
                ...
                if h.ty != quiche::Type::Initial {
                    return;
                }
                (h.version, h.scid.to_vec(), h.dcid.to_vec(), h.token.unwrap_or_default())
            },
            Err(_) => return,
        };

    let Some(odcid) = validate_or_send_retry(
        shard,
        client_addr,
        header_version,
        &header_scid,
    ...
    // The retry'd Initial's DCID is the server SCID we picked when we sent the Retry,
    // so reuse it directly — this also keeps worker affinity intact.
    let server_cid = header_dcid;
...
fn validate_or_send_retry(
    ...
    if !header_token.is_empty() {
        if let Some(odcid) = shard.retry_tokenizer.validate(header_token, client_addr) {
            return Some(odcid);
        }
        log::debug!("rejecting Initial from {client_addr}: invalid retry token");
    }
    send_retry(shard, client_addr, version, header_scid, header_dcid);
    None
}
...
fn send_retry(
    ...
    let new_scid = generate_server_cid_for_worker(shard.worker_id, shard.worker_count);
    let token = shard.retry_tokenizer.mint(client_addr, header_dcid);

    let mut out = [0u8; MAX_DATAGRAM_SIZE];
    let len = match quiche::retry(
        &quiche::ConnectionId::from_ref(header_scid),
        &quiche::ConnectionId::from_ref(header_dcid),
        ...
        Ok(len) => len,
        Err(err) => {
            log::warn!("failed to build Retry packet for {client_addr}: {err:?}");
            return;
        },
    };

    shard.outgoing_datagrams.push_back(PacedDatagram {
        at: Instant::now(),
        to: client_addr,
